...
void MoveTest();
void AssignmentTest();
void DumpAnswer(TreeImpl::Tree &firstTree, TreeImpl::Tree &secondTree, const char *method, const char *treeToDump);
// void getRand (long long n);

TEST(testTree, checkFirstFile)
{
    if (system("./tree < ../tests/1.txt > outTest1.txt"))
        printf("system problem...\n");

    std::ifstream resFile("outTest1.txt");
    std::string res((std::istreambuf_iterator<char>(resFile)), std::istreambuf_iterator<char>());

    std::ifstream answFile("../tests/answ1.txt");
    std::string answ((std::istreambuf_iterator<char>(answFile)), std::istreambuf_iterator<char>());
...
{
    if (system("./tree < ../tests/2.txt > outTest2.txt"))
        printf("system problem...\n");
...
{
    if (system("./tree < ../tests/3.txt > outTest3.txt")) {
        printf("system problem...\n");
    }
